...
def load_shot_images(shot_video, start_secs, end_secs, num_video_frames, video_folder):
    video_path = os.path.join(video_folder, shot_video)
    frames, frame_indices =  decord_video_given_start_end_seconds(video_path, 
                        start_secs=start_secs, end_secs=end_secs,
                        num_video_frames=num_video_frames)
    #print("ICL shot {}".format(frames.shape))
    images =[  Image.fromarray(x).convert('RGB') for x in frames ]
    return images

def get_icl_exmaple(few_shot_annot, N_shot, args, process_images, image_processor, model):
    ICL_images_list = []
    ICL_qs_list = []
    ICL_answer_list = []

    #print(few_shot_annot)
    assert N_shot < len(few_shot_annot), "N_shot {} should be less than the number of shots in few_shot_annot file {}".format(N_shot, len(few_shot_annot))
    # random select N shot from few_shot_annot
    random_idx = np.random.choice(len(few_shot_annot), N_shot, replace=False)
    for i in random_idx:
        shot_annot = few_shot_annot[i]
        shot_qs = shot_annot["conversations"][0]['value']
        shot_answer = shot_annot["conversations"][1]['value']
        shot_video = shot_annot["video"]
        shot_start_secs = shot_annot["start_secs"]
        shot_end_secs = shot_annot["end_secs"]


        shot_images = load_shot_images(shot_video, shot_start_secs, shot_end_secs, args.num_video_frames, args.image_folder)
        shot_images_tensor = process_images(
                    shot_images,
                    image_processor,
                    model.config).to(model.device, dtype=torch.float16)
        ICL_images_list.append(shot_images_tensor)

        n_images = len(shot_images)
        img_placehoder = '<image>\n' * n_images 
        shot_qs = shot_qs.replace("<video>\n", img_placehoder)
        ICL_qs_list.append(shot_qs)
        ICL_answer_list.append(shot_answer)

    ##Print ICL
    #print("============= ICL Shot ============")
    #for i in range(len(ICL_qs_list)):
    #    print("ICL QS:", ICL_qs_list[i])
    #    print("ICL ANS:", ICL_answer_list[i])
    #print("===================================")

    print("Number of ICL Shots: {}".format(len(ICL_qs_list)))
    #print(ICL_qs_list)
    #print(ICL_answer_list)
    #print(ICL_images_list)
    return ICL_images_list, ICL_qs_list, ICL_answer_list

def merge_image_tensor(ICL_images_list, images_tensor):
    new_images_tensor = ICL_images_list
    new_images_tensor.append(images_tensor)
    new_images_tensor = torch.cat(new_images_tensor, dim=0)

    print(new_images_tensor.shape)
...
def main(args):
   ...
   # image_token_se = DEFAULT_IM_START_TOKEN + DEFAULT_IMAGE_TOKEN + DEFAULT_IM_END_TOKEN
    ...
    for line in tqdm(annotations, total=len(annotations)):
        ...
        ## Get An Example from Few-Shot
        ICL_images_list, ICL_qs_list, ICL_answer_list = get_icl_exmaple(few_shot_annot, N_shot, args, process_images, image_processor, model)
        ...
        with torch.inference_mode():
            if args.num_video_frames > 0:
                ...
                #print("QS Image {}".format(images_tensor.shape))
            ...
            # Pre-Pend ICL-Example
            for i in range(len(ICL_images_list)):
                conv.append_message(conv.roles[0], ICL_qs_list[i])
                conv.append_message(conv.roles[1], ICL_answer_list[i])
            new_images_tensor = merge_image_tensor(ICL_images_list, images_tensor)
            #new_images_tensor = images_tensor
