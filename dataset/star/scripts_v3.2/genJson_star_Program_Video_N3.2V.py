...
def format_into_instruct(sample, image_used, qid_to_frames, 
    ...
    from_human_0 = "<video>\n" + "{}\nSelect from options:\n{}".format(program_structured, options)
    from_human_0 = from_human_0 + "Answer: "
...
def main():
    ...
    reverse_order= False
    ver = 3.0
    base_folder = "../sft_annots_video_v3.2"
    input_json_s = [ "star_annotations/STAR_val_NEAT.json", "star_annotations/STAR_train_NEAT.json"]
    #input_json_s = [ "star_annotations/STAR_val_NEAT.json"]

    for input_json in input_json_s:
        output =  get_output_name(base_folder, input_json, image_used, random_order, reverse_order, "Program_Video_v3.2")
