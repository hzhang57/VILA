...
def format_program_string(program_string):
    # Find and add indentation based on parentheses
    indent = 0
    formatted_lines = []
    current_line = ""
    ...
    for char in program_string:
        if char == '(':
            # Add current line to the formatted lines
            if current_line.strip():
                formatted_lines.append(('\t' * indent) + current_line.strip() + ' (')
            else:
            ...
            # Add the current line before closing the parenthesis
            if current_line.strip():
                formatted_lines.append(('\t' * indent) + current_line.strip())
                current_line = ""
            # Decrease the indentation and add closing parenthesis
            indent -= 1
            formatted_lines.append(('\t' * indent) + ')')
        elif char == ',':
            # Add commas to the current line
            current_line += char + ' '
        else:
    ...
    # Join the formatted lines with newline
    return "\n".join(formatted_lines)

## Example usage
#program_string = "Filter_After_Actions(Actions(Situations()):=abc,Unique(Filter_Actions_with_Verb(Actions(Filter_Situations_with_Obj(Situations(),refrigerator):=def),open)))"
#formatted_program = format_program_string(program_string)
...
def convert_program_v6(program, program_middle):
    ...
    for ii, m in enumerate(pg):
        ...
        if m not in MODULES:
            ...
            if argv:
                old_print = "{}({}){}".format(m, argv_lines, "") # No Return
            else:
                old_print = "{}(){}".format(m, "") # No Return
...
def format_into_instruct(sample, image_used, qid_to_frames, 
    ...
    #from_human_0 = "<video>\n" + "STAR Task: Proram " + program_structured + " Select from options: " + options
    from_human_0 = "<video>\n" + "STAR Task: Proram {} Select from options: {}".format(program_structured, options)
...
def main():
    ...
    for input_json in input_json_s:
        output =  get_output_name(base_folder, input_json, image_used, random_order, reverse_order, "Program_Video_v3.0")
