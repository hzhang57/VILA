...
def format_into_instruct(sample, image_used, qid_to_frames, 
    ...
    for choice in choices:
        ...
        options.append("({}) {}\n".format(choice['choice_id'], choice['choice']))
        #options.append("{};".format(choice['choice'].strip(".")))
        all_choices.append("({})".format(choice['choice_id']))
        index2ans["({})".format(choice['choice_id'])] = choice['choice'].strip(".").replace("/", ', ')#.lower()
    ...
    from_gpt_1   = "{}\n".format(program_structured)

    from_human_2 = "Select from options:\n{}".format(options)
    from_gpt_2   = "({}) {}".format(answer_id, answer.replace('/', ', '))
    conversation = [
        {
        "from": "human",
        "value": from_human_0
...
def main():
    ...
    reverse_order= False
    ver = 3.0
    base_folder = "../sft_annots_video_v3.3"
    input_json_s = [ "star_annotations/STAR_val_NEAT.json", "star_annotations/STAR_train_NEAT.json"]

    for input_json in input_json_s:
        output =  get_output_name(base_folder, input_json, image_used, random_order, reverse_order, "Query_Gen_Program_3round_Video_v3.3")
